...
# Try to load model
model_paths = [
    'models/healthcare_ensemble_model_fixed.pkl',
    'healthcare_ensemble_model_fixed.pkl',
    'models/healthcare_ensemble_model.pkl',
]
...
generalized_analyzer = create_enhanced_analyzer(ensemble, vectorizer)
...
def upload():
    ...
    for review in df['review'].head(50):
        review = str(review)
        if review.strip():
            result_text, _ = predict_sentiment(review)
